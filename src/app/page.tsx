...
function distributeValues(values: number[], desiredSum: number): number[] {
  ...
  let currentSum = integerParts.reduce((a, b) => a + b, 0);
  let remainder = desiredSum - currentSum;
...
export default function Home() {
    ...
    const interval = setInterval(() => {
      setGrid(prev => prev.map((row, i) => 
        row.map((cellState, j) => {
          ...
          if (cellState === "young") {
            return adultNeighbors >= 3 ? "adult" : "off";
          } 
          if (cellState === "adult") {
            return youngNeighbors === 3 ? "adult" : "off";
          } 
          if (cellState === "elder") {
            return youngNeighbors === 3 ? "adult" : "elder";
          }
          if (cellState === "off" && elderNeighbors >= 3) {
            return "young";
          }
  ...
  const randomizeGrid = () => {
    const newGrid = Array(GRID_SIZE).fill(null).map(() => 
      Array(GRID_SIZE).fill(null).map(() => {
        let total = 0;
        for (const state of STATES) total += percentages[state];
        if (total === 0) return "off";
        
        const random = Math.random() * total;
        let sum = 0;
        for (const state of STATES) {
          sum += percentages[state];
  ...
  const handlePercentageChange = (state: string, value: number) => {
    setPercentages(prev => {
      const newValue = Math.max(0, Math.min(100, value));
      const oldValue = prev[state];
      ...
      if (originalOtherSum === 0) {
        ...
        let remainder = newRemaining % otherStates.length;
          ...
          {STATES.map((state) => (
            <div key={state} className="flex flex-col gap-2">
              ...
              <input
                ...
                min="0"
                max="100"
                value={percentages[state]}
                onChange={(e) => handlePercentageChange(state, parseInt(e.target.value))}
                className="cursor-pointer"
              />
              <span className="text-xs text-gray-300">{percentages[state]}%</span>
            </div>
          ))}
        </div>
